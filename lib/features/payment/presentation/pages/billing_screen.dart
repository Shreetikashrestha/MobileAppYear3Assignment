...
class _BillingScreenState extends ConsumerState<BillingScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Billing & Payments', style: TextStyle(color: Colors.black)),
      ...
      data: (transactions) {
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              _buildSummaryCard('Total Spent', 'NPR ${totalSpent.toStringAsFixed(2)}', Colors.purple),
        ...
        children: [
          walletAsync.when(
            data: (wallet) {
              return Column(
                children: [
                  _buildSummaryCard('Available Balance', 'NPR ${wallet['currentBalance']?.toStringAsFixed(2) ?? '0.00'}', Colors.green),
                  const SizedBox(height: 10),
                  Row(
                    children: [
                      Expanded(child: _buildSummaryCard('Total Earnings', 'NPR ${wallet['totalEarnings']?.toStringAsFixed(2) ?? '0.00'}', Colors.blue, small: true)),
                      const SizedBox(width: 10),
                      Expanded(child: _buildSummaryCard('Pending', 'NPR ${wallet['pendingPayouts']?.toStringAsFixed(2) ?? '0.00'}', Colors.orange, small: true)),
                    ],
                  ),
                  const SizedBox(height: 20),
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: () => _showPayoutDialog(wallet['currentBalance']?.toDouble() ?? 0.0),
                      style: ElevatedButton.styleFrom(
                        backgroundColor: Colors.purple,
                        padding: const EdgeInsets.symmetric(vertical: 16),
                        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                      ),
                      child: const Text('Request Payout', style: TextStyle(fontSize: 16)),
                    ),
                  ),
                ],
              );
  ...
  Widget _buildSummaryCard(String title, String value, Color color, {bool small = false}) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(title, style: TextStyle(color: color, fontWeight: FontWeight.bold, fontSize: small ? 14 : 16)),
          const SizedBox(height: 8),
          Text(value, style: TextStyle(color: color, fontWeight: FontWeight.bold, fontSize: small ? 18 : 24)),
        ],
      ),
    );
  }
  ...
  Widget _buildTransactionList(List<TransactionEntity> transactions) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final isCredit = t.type == 'payment' || t.type == 'refund'; // For brand, payment is debit. For influencer, payment is credit.
        // Actually, logic depends on role.
        ...
        return Card(
          ...
          child: ListTile(
            ...
            title: Text(t.type.toUpperCase(), style: const TextStyle(fontWeight: FontWeight.bold)),
        ...
        child: Column(
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              value: method,
              items: ['Bank Transfer', 'eSewa', 'Khalti'].map((m) => DropdownMenuItem(value: m, child: Text(m))).toList(),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () {
                    ...
                    ref.read(paymentViewModelProvider.notifier).requestPayout(amount: amount, method: method);
                },
                style: ElevatedButton.styleFrom(
                  backgroundColor: Colors.purple,
                  padding: const EdgeInsets.symmetric(vertical: 16),
